  ...
  <main className="flex-1 p-6">
    {Order.length === 0 ? (
      ...
      <div className="space-y-2">
        {Order.map((item) => (
          <div key={item.id} className="bg-white rounded-xl shadow p-4 flex gap-6 items-center">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-600 mt-1">Rating: {item.rate}</div>
              <div className="text-sm text-gray-600 mt-1">Quantity: {item.count}</div>
              <div className="text-sm text-gray-600 mt-1">Price: ${item.count*item.price}</div>
              <button onClick={()=>handleDelete(item.id)} className="bg-red-600 hover:bg-red-700 text-white py-2 px-2 rounded-lg">
                Cancel Order</button>
            </div>
          </div>
        ))}
